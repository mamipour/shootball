...
func _on_multi_shots_received(p1_shots: Array, p2_shots: Array):
	...
	for p in my_pills:
		p.is_selected = false
	selected_pill = null
	print("[OnlineBattle] Multi-shots fired")
...
func _update_hud():
	var alive_my := _alive_pills(my_pills).size()
	var alive_opp := _alive_pills(opp_pills).size()
	status_label.text = "YOU: %d units   —   OPP: %d units" % [alive_my, alive_opp]
	...
	match phase:
		...
		Phase.AIMING:
			...
			if shots_submitted:
				...
				var alive_count: int = _alive_pills(my_pills).size()
				phase_label.text = "Tap a unit to aim  ·  %d/%d aimed  ·  push enemies into the pit!" % [locked_count, alive_count]
...
func _build_hud():
	...
	status_label.position = Vector2(vp.x / 2.0 - 200, 8)
