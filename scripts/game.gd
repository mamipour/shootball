...
func _start_round():
	...
	for p in player_pills:
		p.is_selected = false
	print("[Round %d] Aiming phase started" % round_num)
...
func _ai_decide():
	...
	print("[AI-%s] pill %d, power %.0f, score=%.1f" % [
		["Easy","Normal","Hard"][diff], ai_shot_idx, ai_shot_power, best_score])
...
func _fire_shots():
	...
	if aim_locked and aim_valid and sel_idx >= 0:
		...
		print("[FIRE] Player pill %d, impulse %s" % [sel_idx, str(impulse)])

	if ai_decided and ai_shot_idx >= 0:
		var impulse: Vector2 = ai_shot_dir * ai_shot_power
		ai_pills[ai_shot_idx].kick(impulse)
		print("[FIRE] AI pill %d, impulse %s" % [ai_shot_idx, str(impulse)])
	else:
		print("[FIRE] AI did NOT shoot. ai_decided=%s ai_shot_idx=%d" % [
			str(ai_decided), ai_shot_idx])
...
func _end_round():
	if goal_this_round:
		...
		print("[Round %d] No goal, next round" % round_num)
		_start_round()
...
func _update_hud():
	...
	# Debug line — shows AI state in bottom-left corner
	if debug_label:
		debug_label.text = "phase=%s  ai_decided=%s  ai_pill=%d" % [
			Phase.keys()[phase], str(ai_decided), ai_shot_idx]
...
func _build_hud():
	...
	debug_label.position = Vector2(10, 695)
	debug_label.size = Vector2(500, 20)
	debug_label.add_theme_font_size_override("font_size", 12)
	debug_label.add_theme_color_override("font_color", Color(1, 1, 1, 0.35))
