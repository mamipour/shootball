...
enum Phase { WAITING, AIMING, EXECUTING, POINT_SCORED, GAME_OVER }
...
func _set_positions(frame: Array):
	...
	if frame.size() > 6:
		ball.position = origin + Vector2(float(frame[6][0]), float(frame[6][1]))
...
func _on_replay_complete():
	...
	if is_game_over:
		...
		var i_scored := (scorer == Online.user_id)
